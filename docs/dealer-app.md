...
**Registration correction:** Live dashboard inspection found that organization `221105492` is BHS's merchant organization. Both app registrations created there are merchant-owned and lack external distribution controls; they are not usable as the proposed SSA installation or public App Store registration. Re-register the same code under a Shopify Partner organization before dealer distribution. The BHS login has no existing Partner organization; its signup form is prepared but the Partner Program Agreement is not accepted. The existing hosted code/database work remains reusable. See [SSA pilot setup](dealer-app-ssa-pilot.md).
...
- Register apps under a Shopify Partner organization for external merchant distribution. The current merchant-owned registrations cannot provide the planned dealer install links. A public app distribution route is required for the shared app used by unrelated dealers; a separate Partner-owned custom app supports SSA's single-store pilot. Limited listing visibility does not bypass Shopify review. [Distribution](https://shopify.dev/docs/apps/launch/distribution)
