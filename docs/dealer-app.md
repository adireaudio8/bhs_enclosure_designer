...
The initial app implementation is hosted for setup and review. It is **not accepting installations or customer orders**.

Registered Shopify app: **BHS Dealer Enclosure Designer**, client ID `0683dc0526169d9bd139c17b54f2a20d`, in the Basshead Supply organization. Its configuration and theme extension validate. Separate hosting project: `bhs-dealer-enclosure-designer`, at https://bhs-dealer-enclosure-designer.vercel.app/dealer. The SQL migration has not been applied to a hosted database. No dealer has been selected for the pilot.

Shopify version `dealer-foundation-20260919` is uploaded **without release**: https://dev.shopify.com/dashboard/221105492/apps/425553231873/versions/1135543746561. The isolated hosting deployment `dpl_7egrjcdG8z8ttaPXAZAqgBJTUMdb` is READY and its browser page confirms setup is in progress. Vercel labels the new project's default deployment production; this is separate from the existing BHS storefront, and dealer installations/orders remain disabled. Live unauthenticated session, proxy, catalog and webhook requests return 401.

The new app's own Shopify credentials and session/encryption/operator keys are configured on its separate hosting project. Its public Supabase URL points to the same pricing project, `ftvfqjqgkrnoxeystorg`. The required server key is not configured: a bulk production-environment export was rejected by automatic approval review because it would include unrelated secrets. A narrower metadata/read attempt retrieved only the database URL; Vercel would not return the existing sensitive server key. Do not work around that protection. Prepare a dedicated revocable server key for this app with Andrew's approval, then apply the tested migration and finish integration testing. No BHS production credential was copied.
...
3. Apply `sql/20260919_dealer_designer.sql` once to the intended Supabase project after reviewing it. This creates private `bhs_dealer_*` tables and service-role-only functions. It does not edit products, existing pricing or Collective settings.
