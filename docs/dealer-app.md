...
The SSA pilot is deployed, its Shopify version is released, and its private installation link is ready for SSA's owner or administrator. The database is connected and installation preflight passes. No dealer is installed, and **customer ordering is disabled**.

**Registration correction resolved for SSA:** The original registrations under merchant organization `221105492` cannot distribute to external dealers and remain unused. With Andrew's explicit agreement approval, the free Basshead Supply Partner organization `5196649` (Dev Dashboard `236496624`) was created. Its new **BHS Dealer SSA Pilot** registration now has custom distribution restricted to SSA. A future shared public app still needs a separate Partner-owned registration and Shopify review. See [SSA pilot setup](dealer-app-ssa-pilot.md).
...
Active Shopify app: **BHS Dealer SSA Pilot**, client ID `e819030eaa28f090db1c2a659c5b9f62`, app ID `425584328705`, in the new Basshead Supply Partner organization. Its configuration and theme extension validate. The existing isolated hosting project, `bhs-dealer-enclosure-designer`, at https://bhs-dealer-enclosure-designer.vercel.app/dealer was rebound to this app; the original BHS designer deployment is unchanged. The SQL migration remains applied to the pricing database. Andrew selected **Sound Solutions Audio** and confirmed its existing Collective membership. Its canonical store is `soundsolutionsaudio.myshopify.com`.

Shopify version [`ssa-pilot-20260919`](https://dev.shopify.com/dashboard/236496624/apps/425584328705/versions/1135622160385) is released. Hosting deployment `dpl_7wfAMbwy6hQqixGWdKYy9eNAwKwm` is READY from code commit `d20e907`. `DEALER_ALLOWED_SHOPS` restricts all dealer access to SSA. Live SSA installation preflight reaches its correct OAuth host/client ID with 307; another store is rejected with 403. Unauthenticated session, proxy, catalog and webhook requests return 401. No dealer is installed or activated. Legacy merchant-owned versions are historical, not the active distribution.
...
The additive migration created five private tables and three functions. Setup verification confirmed RLS on all five tables, no table/function grants for `anon`, `authenticated` or `PUBLIC`, and zero installed dealers/quotes. The authenticated operator catalog returned 200 from live pricing with **4,818 candidate price options across 49 groups** of at most 100 variants; this is a preparation manifest, not an approved or published Collective catalog. Review and narrow the supported first-release options before creating products. Installation preflight did not follow OAuth or grant store access.
...
- Register apps under a Shopify Partner organization for external merchant distribution. SSA's new Partner-owned custom app now supplies its single-store installation link; the legacy merchant-owned registrations cannot. A public app distribution route is required for the shared app used by unrelated dealers. Limited listing visibility does not bypass Shopify review. [Distribution](https://shopify.dev/docs/apps/launch/distribution)
...
2. Private environment settings are documented in `.env.example`: shared pricing Supabase URL/server key, the dealer app key/secret, `DEALER_APP_URL`, session/operator secrets and a 32-byte random base64 encryption key. SSA's deployment also sets `DEALER_ALLOWED_SHOPS=soundsolutionsaudio.myshopify.com`. Do not remove the store restriction during this private pilot. Do not copy BHS-only Shopify credentials or commit secrets. Only one app registration may own a shop record in the current database; plan migration and webhook isolation before adding another registration.
...
4. The active dedicated hosting URL is configured in `dealer-app/shopify.app.bhs-dealer-ssa-pilot.toml`. Validate from the repo root with `shopify app config validate --path ./dealer-app --config bhs-dealer-ssa-pilot --json`. Deploy that named app configuration from `dealer-app`; never deploy the root BHS app or legacy merchant-owned configuration as a substitute.
5. SSA custom distribution is configured and its restricted link is ready. For broader distribution, finish the separate public-app registration, privacy policy, support/contact information, protected order-data access requirements and Shopify review. A released private pilot is not an approved app listing.
...
10. The dealer duplicates the current theme, then adds the BHS Custom Enclosures block to a page in the unpublished copy for the controlled pilot. This release supports the primary storefront domain or myshopify domain, USD and normal locale paths; secondary market domains/currencies need separate support.
...
The 74 automated tests pass, including PostgreSQL migration execution in an in-memory PGlite database, quote idempotency, tenant checks, exact pilot store restrictions, private table/function permissions, and versioned order reconciliation. Shopify config validation, theme-extension checks and pilot release pass. Engine parity, TypeScript and the production build pass. Next.js is patched to 16.3.5 and Shopify CLI to 4.8.0; the updated lockfile's dependency audit reported zero vulnerabilities. Real Shopify installation, supplier-side property transfer, automatic payments, production reconciliation and live checkout remain pilot/launch checks.
